...
def simulateDistanceTraveled(ienv, w, phase):
    ...
    print('%4.2f %4.2f: %7.4f, [%6.3f, %6.3f]' %(w, phase, distance_traveled, diff[0], diff[1]))
    sys.stdout.flush()
...
if __name__ == '__main__':
    NUM_LINKS = 10
    env = SimpleSnakeEnv(num_links=NUM_LINKS, controller=SimpleSinusoidController(NUM_LINKS))
    all_distances = [
        (simulateDistanceTraveled(env, w, phase), w, phase)
        for w in np.linspace(0, pi/4, 20) for phase in np.linspace(0, pi/4, 20)
    ]
